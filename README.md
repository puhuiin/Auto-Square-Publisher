...
- 🔥 **重磅热点价值打分算法 (Impact Scorer)**：
  - 引入市场冲击力关键词加权算法（ETF、SEC、降息、上线、Launchpool、爆仓、突破等），优先发布最具吸睛力的新闻。
...
- 🕘 **活跃时段窗口（可选）**：通过 `ACTIVE_HOURS_BEIJING=8-23` 限制仅在北京时间 8-23 点发帖，避开低流量深夜时段，保持账号互动权重。
...
- **运维调优参数**：在仓库 **Settings → Secrets and variables → Actions → Variables** 中新增以下变量即可生效（全部无需改代码）：
  ...
  | `ACTIVE_HOURS_BEIJING` | 空 | 北京时间活跃窗口，例 `8-23` 表示仅北京 8-23 点发帖（空 = 全天） |
- **CI 回归防线**：`tests/test_core.py` 内置 38 个离线回归测试，`.github/workflows/ci.yml` 在每次 push/PR 时自动编译并跑测试，防止守护逻辑被后续改动悄悄破坏。
